...
class Preprocess4Pretrain(Pipeline):
    ...
    def __call__(self, instance):
        ...
        masked_word_ids, masked_pos_ids, masked_dep_ids = self.indexer(masked_word_tokens, masked_pos_tokens, masked_dep_tokens)
...
def main(train_cfg='config/pretrain.json',
    ...
    if mode == 'train':
        def get_loss(model, batch, global_step): # make sure loss is tensor
            ...
            loss_pos = criterion1(logits_pos.transpose(1, 2), masked_pos_ids) # for masked pos
            loss_pos = (loss_pos*masked_weights.float()).mean()

            loss_dep = criterion2(logits_dep.transpose(1, 2), masked_dep_ids) # for masked dep
